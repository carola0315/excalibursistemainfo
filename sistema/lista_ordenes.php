...
use Mpdf\Language\ScriptToLanguage;
    ...
    $query = mysqli_query($conection, "SELECT DISTINCT COUNT(fecha_evento) FROM orden_servicio GROUP BY fecha_evento LIMIT 1");
            
        $result = mysqli_fetch_array($query);
        
            if($result >= 3){

                //<ScriptToLanguagealert("TIENE 2 EVENTOS EL MISMO DÍA");

                echo '<script language="javascript">alert("ALERTA!! TIENE 3 EVENTOS EL MISMO DÍA");</script>';
...
<body>
	...
	<section id="container">
		...
		<h1><i class="fas fa-list"></i>LISTA DE ORDENES DE SERVICIO</h1>

        <form action = "buscar_ordenes.php" method = "get" class = "buscador">
            <i class="fas fa-search"></i><input type = "text" name = "busqueda"  id= "busqueda" placeholder="N° Orden">
       ...
       <!-- <div>
        ...
        <table>
            <tr>
                ...
                <th>Comercial</th>
                <th>Estado</th>
                <th class = "textright">Total Orden Servicio</th>
        ...
        <?php
            ...
            $query_ordenes = mysqli_query($conection, "SELECT o.id_Orden, o.id_cotizacion, o.id_cliente, o.fecha_evento, c.tipo_servicio, c.Usuario as comercial, o.estatus, o.Precio_total, cl.Nombre_cliente
                                                            ...
                                                            ORDER BY o.fecha_evento AND o.estatus ASC
                                                            LIMIT $desde, $por_pagina");

            mysqli_close($conection);

            $result = mysqli_num_rows($query_ordenes);
            
            if($result > 0){

                while ($data = mysqli_fetch_array($query_ordenes)) {
                    if($data["estatus"] == 0){
